...
class SummarizationModel(object):
  ...
  def _add_placeholders(self):
    ...
    self._rel_scores_batch = tf.placeholder(tf.float32, [hps.batch_size.value, None], name='rel_scores_batch')
    ...
    # decoder part
    if hps.mode.value == "train" or hps.mode.value == "eval":
      ...
      self._padding_mask = tf.placeholder(tf.float32, [hps.batch_size.value, hps.max_dec_steps.value], name='padding_mask')
    ...
    if hps.mode.value == "decode":
      ...
      self._padding_mask = tf.placeholder(tf.float32, [hps.batch_size.value, hps.max_dec_steps], name='padding_mask')
  ...
  def _make_feed_dict(self, batch, just_enc=False):
    ...
    feed_dict[self._rel_scores_batch] = batch.scores
    if FLAGS.pointer_gen:
  ...
  def _add_decoder(self, inputs):
    ...
    outputs, out_state, attn_dists, p_gens, coverage = attention_decoder(inputs, self._dec_in_state, self._enc_states, self._rel_scores_batch, cell, initial_state_attention=(hps.mode.value=="decode"), pointer_gen=hps.pointer_gen.value, use_coverage=hps.coverage.value, prev_coverage=prev_coverage)
  ...
  def _add_seq2seq(self):
    ...
    if hps.mode.value == "decode":
      ...
      self._topk_log_probs, self._topk_ids = tf.nn.top_k(log_dists, hps.batch_size.value*2) # note batch_size=beam_size in decode mode
  ...
  def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage):
    ...
    batch.scores = np.array(batch.scores, dtype=np.float32)
    feed = {
        self._rel_scores_batch: batch.scores,
