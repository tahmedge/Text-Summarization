...
tf.app.flags.DEFINE_integer('max_enc_steps', 80, 'max timesteps of encoder (max source text tokens)')
...
tf.app.flags.DEFINE_integer('vocab_size', 34398, 'Size of vocabulary. These will be read from the vocabulary file in order. If the vocabulary file contains fewer words than this number, or if this number is set to 0, will take all words in the vocabulary file.')
...
def setup_training(model, batcher):
  ...
  with default_device:
    ...
    if FLAGS.restore_best_model:
      restore_best_model()
    saver = tf.train.Saver(max_to_keep=3) # only keep 1 checkpoint at a time
...
def run_eval(model, batcher, vocab):
  ...
  while True:
    ...
    #print(summaries)
    #tf.logging.info('summaries: %f', summaries)
...
def main(unused_argv):
  ...
  if hps.mode.value == 'train':
    ...
    now = datetime.now()
    print("now =", now)
    d1 = now.strftime("%d_%m_%Y_%H:%M:%S")
    decoded_file=open(str(d1)+"_decoded_file.txt","w+")
    ref_file=open(str(d1)+"_reference_file.txt","w+")
    decoder.decode(decoded_file,ref_file,) # decode indefinitely (unless single_pass=True, in which case deocde the dataset exactly once)
  else:
