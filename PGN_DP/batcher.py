...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, rel_scores, vocab, hps):
    ...
    if len(article_words) > hps.max_enc_steps.value:
      ...
      rel_scores = rel_scores[:hps.max_enc_steps.value]
  ...
  def pad_encoder_input(self, max_len, pad_id):
    ...
    while len(self.enc_input) < max_len:
      ...
      self.rel_scores=np.append(self.rel_scores, [0.0])
      #self.rel_scores.append(0.)
      #self.rel_scores.append(1.) #TODO: REMOVE!!!!!
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    self.scores = np.zeros((hps.batch_size.value, max_enc_seq_len), dtype=np.float32)
    # Fill in the numpy arrays
    for i, ex in enumerate(example_list):
      #print(float(ex.rel_scores[:]))
      self.scores[i, :] = ex.rel_scores[:]
      #print(self.scores[i, :])
  ...
  def init_decoder_seq(self, example_list, hps):
    ...
    self.padding_mask = np.zeros((hps.batch_size.value, hps.max_dec_steps.value), dtype=np.float32)
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      ...
      example = Example(article, abstract, rel_scores, self._vocab, self._hps) # Process into an Example.
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      try:
        ...
        relevancy_score =  e.features.feature['relevancy'].bytes_list.value[0]
        relevancy_score=relevancy_score.decode("utf-8")

        relevancy_score=relevancy_score.split(', ')
        #print("article")
        #print(article_text)
        #print("abstract")
        #print(abstract_text)
        ...
        #print(article_text)
        #relevancy_score=str(relevancy_score[1:-1])
        #for s in relevancy_score:
        #  print(s)

        #relevancy_score = np.array([np.float32(s) for s in str(relevancy_score).split(', ')], dtype=np.float32)
        relevancy_score = np.array([np.float32(s) for s in relevancy_score], dtype=np.float32)
      ...
      if len(article_text)==0: # See https://github.com/abisee/pointer-generator/issues/1
        ...
        yield (article_text, abstract_text, relevancy_score)
